...
sql320 = "select date1, ac, sn_u from a320_fault_massages " + \
         "where sn_u is not null and sn_u != '' and sn_u != 'N/A' " + \
         "order by date1 desc"
sql330 = "select date1, ac, sn_u from a330_fault_massages " + \
         "where sn_u is not null and sn_u != '' and sn_u != 'N/A' " + \
         "order by date1 desc"
...
dfac = pd.concat([df320, df330])
dfac.sort_values(by=['date1'], ascending=False)
dfsql = dfac.drop('sn_u', axis=1).join(dfac['sn_u']
...
dfsql.drop_duplicates(subset='sn_u', inplace=True)

dfexcel = pd.read_excel('E:\\development\\PythonForDataAnalysis\\sql\\input.xlsx', usecols=[2], dtype=str)

res = pd.merge(dfexcel, dfsql, left_on='序号', right_on='sn_u', how='left')
res = res.replace('nan', '')
